...
int main(int argc, char* argv[]) {
    ...
    //tror det kan vara fel med intargs och stringargs, kolla memoryserver..
    
    string str;
    int arg;
    while(cin >> str){ //måste kolla sen om saker från kommandot innan ligger kvar
        try{
            arg = switchStr(str);
            switch(arg){
                case 1:
                {
                    ...
                    if(str == "Newsgroups"){
                        ...
                        mh.transmit(conn);
                        mh = Message(conn);
                        for(int i = 0; i < mh.stringargs.size(); ++i){ //måste jag skriva mh.stringargs?
                            cout << mh.intargs[i+1] << ". " << mh.stringargs[i] << endl;
                        }
                    }else if(str == "Articles"){
                        string IDNbr;
                        IDNbr = "-1";
                        cin >> IDNbr; //göra om IDNbr to int stoint
                        if(IDNbr == "-1"){
                            help();
                        }else{
                            vector<int> intargs;
                            intargs.push_back(stoi(IDNbr)); //måste vi kolla om det blir en siffra?
                            ...
                            }else if(mh.ack == Protocol::ANS_ACK){ //se om det är acknowledge
                ...
                case 2:
                {
                    ...
                    if(str == "Newsgroup"){
                        ...
                        if(text == "-1"){
                            ...
                            mh.transmit(conn);
                            mh = Message(conn);
                            if(mh.intargs[0] == Protocol::ERR_NG_DOES_NOT_EXIST){  //Ändra fel
                                cout << "The Newsgroup does not exist." << endl;
                            }else{
